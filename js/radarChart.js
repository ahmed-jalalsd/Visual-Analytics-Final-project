...
// }
	...
	if (data == null || data.length == 0) {
		...
		for (i = 0; i < data.length; i++) {
			...
			out.push([{
				axis: "World Rank",
				value: 100 - output[i][0] * 100 / 400
			},
			{
				axis: "Teaching Environmemt",
				value: output[i][3]
			},
			{
			...
			{
				axis: "Research Excellance",
				value: output[i][5]
			},
			{
				axis: "Citation (Research Influence)",
				value: output[i][6]
			}
		...
		//var allAxis = d3.keys(data[0])
		total = allAxis.length; //The number of different axes
		//console.log(total);
		// to change the size of the circle added today
		...
		var svg = d3.select(id).append("svg")
			.attr("width", cfg.w + cfg.margin.left + cfg.margin.right + 30)
			.attr("height", cfg.h + cfg.margin.top + cfg.margin.bottom + 30)
			.attr("class", "radar" + id);
		//Append a g element		
		var g = svg.append("g")
			.attr("transform", "translate(" + (cfg.w / circleTransX + cfg.margin.left) + "," + (cfg.h / circleTransY + cfg.margin.top) + ")");
		...
		blobWrapper
			...
			.on('mouseover', function (d, i) {
				...
				for (ix = 0; ix < output.length; ix++) {
					if (4 * (100 - vegeta[0][1]) == output[ix][0]) {
						goku = output[ix][1];
					}
				}
				p = $("path")
